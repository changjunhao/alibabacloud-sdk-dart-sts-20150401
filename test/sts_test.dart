...
void main() {
  group('STS Models Tests', () {
    ...
    test('AssumeRoleResponse fromJson', () {
      ...
      expect(response.assumedRoleUser?.arn, equals('acs:ram::123456789012****:assumed-role/test/session'));
      expect(response.assumedRoleUser?.assumedRoleId, equals('test-assumed-role-id'));
      expect(response.credentials?.accessKeyId, equals('test-access-key-id'));
      expect(response.credentials?.accessKeySecret, equals('test-access-key-secret'));
      expect(response.credentials?.securityToken, equals('test-security-token'));
    ...
    test('AssumeRoleWithOIDCRequest toJson', () {
      ...
      expect(json['DurationSeconds'], equals(3600));
      expect(json['OIDCProviderArn'], equals('acs:ram::123456789012****:oidc-provider/test'));
    ...
    test('AssumeRoleWithSAMLRequest toJson', () {
      ...
      expect(json['SAMLProviderArn'], equals('acs:ram::123456789012****:saml-provider/test'));
    });
